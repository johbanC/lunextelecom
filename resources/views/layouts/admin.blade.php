...
<head>
    ...
    <title>@yield('title', 'Panel Admin') - Lunex Telecom</title>
    ...
    <nav class="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-gray-200 shadow-sm">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
            <a href="{{ route('admin.agreements.index') }}" class="flex items-center gap-3">
                ...
                <span class="hidden sm:block font-semibold text-gray-500 text-sm tracking-wide">Formularios</span>
            </a>
            <div class="flex items-center gap-3">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-gray-600 hover:bg-gray-50">Mi perfil</a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full text-left px-4 py-2 text-brand-red hover:bg-brand-red-50">Cerrar sesión</button>
                        </form>
                    </div>
                </div>
            </div>
