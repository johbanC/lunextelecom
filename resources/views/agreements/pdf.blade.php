...
<body>
    ...
    <div class="signature-box">
        <div style="font-weight: bold; color: #0e57a4; text-transform: uppercase; margin-bottom: 4px;">Signature</div>
        @if ($signatureDataUri)
            <img src="{{ $signatureDataUri }}" alt="Firma">
        @endif
        <div style="font-size: 9px; color: #555; margin-top: 4px;">Firmado el {{ $agreement->signed_at->format('d/m/Y H:i') }}@if ($agreement->signed_ip) desde IP {{ $agreement->signed_ip }} @endif</div>
    </div>

    <div style="font-size: 8px; color: #999; margin-top: 10px; text-align: center;">
        Enlace generado por {{ $agreement->creator?->name ?? '—' }} el {{ $agreement->created_at->format('d/m/Y H:i') }}
    </div>
</body>
</html>
