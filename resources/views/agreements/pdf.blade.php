...
<body>
    ...
    <div class="signature-box">
        ...
        @endif
        <div style="font-size: 9px; color: #555; margin-top: 4px;">{{ __('Signed on :date', ['date' => $agreement->signed_at->format('d/m/Y H:i')]) }}@if ($agreement->signed_ip) &nbsp;{{ __('from IP :ip', ['ip' => $agreement->signed_ip]) }}@endif</div>
    </div>

    <div style="font-size: 8px; color: #999; margin-top: 10px; text-align: center;">
        {{ __('Link generated by :name on :date', ['name' => $agreement->creator?->name ?? '—', 'date' => $agreement->created_at->format('d/m/Y H:i')]) }}
    </div>
</body>
</html>
