...
@section('content')
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        ...
        <div class="rounded-2xl border border-gray-300 bg-gray-100 p-5 shadow-sm">
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Vencidos</p>
            <p class="mt-1 text-3xl font-bold text-gray-700">{{ $counts['expired'] }}</p>
        </div>
        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
            <p class="text-xs font-semibold uppercase tracking-wide text-emerald-600">Firmados</p>
    ...
    <div class="flex items-center justify-between flex-wrap gap-4 mb-4">
        <div class="inline-flex flex-wrap p-1 rounded-full bg-gray-100 border border-gray-200">
            @foreach (['all' => 'Todos', 'pending' => 'Pendientes', 'expired' => 'Vencidos', 'signed' => 'Firmados'] as $key => $label)
        ...
        <table class="w-full text-left text-sm min-w-[900px]">
            <thead>
                <tr class="border-b border-gray-200 text-xs uppercase tracking-wide text-gray-400">
                    ...
                    <th class="p-4 font-semibold">Negocio / Cliente</th>
                    <th class="p-4 font-semibold">Estado</th>
                    <th class="p-4 font-semibold">Creado</th>
                    <th class="p-4 font-semibold">Firmado</th>
                        ...
                        <td class="p-4">
                            @if ($agreement->isSigned())
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold">
                                    <span class="size-1.5 rounded-full bg-emerald-500"></span> Firmado
                                </span>
                            @elseif ($agreement->isExpired())
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-200 text-gray-600 text-xs font-bold">
                                    <span class="size-1.5 rounded-full bg-gray-500"></span> Vencido
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold">
                                    <span class="size-1.5 rounded-full bg-amber-500"></span> Pendiente
                                </span>
                                <div class="text-xs text-gray-400 mt-1">
                                    {{ $agreement->expires_at ? 'Vence ' . $agreement->expires_at->format('d/m/Y') : 'Sin vencimiento' }}
                                </div>
                            @endif
                        </td>
                        <td class="p-4 text-gray-500">
                            {{ $agreement->created_at->format('d/m/Y H:i') }}
                            <div class="text-xs text-gray-400">por {{ $agreement->creator?->name ?? '—' }}</div>
                        </td>
                        <td class="p-4 text-gray-500">{{ $agreement->signed_at?->format('d/m/Y H:i') ?? '—' }}</td>
                        <td class="p-4 text-right space-x-3 whitespace-nowrap">
                            <a href="{{ route('admin.agreements.show', $agreement) }}" class="text-brand-blue font-semibold hover:text-brand-blue-700 hover:underline">Ver</a>
                            @if ($agreement->isSigned())
                ...
                @empty
                    <tr>
                        <td colspan="7" class="p-10 text-center text-gray-400">
                            ...
                            No hay formularios en esta vista.
                        </td>
                    </tr>
                @endforelse
            </tbody>
