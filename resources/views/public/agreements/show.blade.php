...
<body class="min-h-screen bg-gray-100 bg-[radial-gradient(circle_at_top,_var(--color-brand-blue-50),_var(--color-gray-100)_60%)] p-4 md:p-10 font-sans text-gray-800">
    ...
    <div class="max-w-3xl mx-auto rounded-2xl bg-white shadow-xl shadow-gray-300/40 overflow-hidden">
        ...
        @if ($agreement->isSigned())
            <div class="p-10 text-center space-y-3">
                ...
                <p class="text-lg font-bold text-gray-800">Este formulario ya fue firmado.</p>
                <p class="text-sm text-gray-500">Firmado el {{ $agreement->signed_at->format('d/m/Y H:i') }}. Si crees que esto es un error, contacta a Lunex Telecom.</p>
            </div>
        @elseif ($expired)
            <div class="p-10 text-center space-y-3">
                ...
                <p class="text-lg font-bold text-gray-800">Este enlace para firmar ha vencido.</p>
                <p class="text-sm text-gray-500">Contacta a Lunex Telecom para que te envíen un nuevo enlace.</p>
            </div>
        @else
            <form id="agreementForm" method="POST" action="{{ route('public.agreements.store', $agreement->uuid) }}" class="p-5 md:p-8 space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <div class="flex items-center gap-2 rounded-full bg-gray-100 border border-gray-200 pl-3 pr-4 py-1.5">
                        <span class="text-xs font-bold uppercase tracking-wide text-gray-500">Fecha</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Owner's Name</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Primary Phone Number</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Business Name</label>
                        <input type="text" name="business_name" value="{{ old('business_name') }}" required
                            class="w-full h-11 border border-gray-300 rounded-lg px-3 uppercase font-medium focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Location Address</label>
                        <input type="text" name="address" value="{{ old('address') }}" required
                            class="w-full h-11 border border-gray-300 rounded-lg px-3 uppercase font-medium focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Last 4 Digits of Bank Acct</label>
                        <input type="text" name="last_4_bank" value="{{ old('last_4_bank') }}" required maxlength="4" pattern="\d{4}"
                            class="w-full h-11 border border-gray-300 rounded-lg px-3 font-medium focus:ring-2 focus:ring-brand-blue focus:border-brand-blue outline-none transition">
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1.5">Training Date</label>
                ...
                <div class="flex items-start gap-3 rounded-xl border border-brand-red-50 bg-brand-red-50/60 border-l-4 border-l-brand-red p-4">
                    ...
                    <p class="text-sm font-semibold italic text-brand-red-600 flex items-center flex-wrap gap-1">
                        <span>I authorize Lunex Telecom, Inc. to initiate debit ACH to the account ending with</span>
                        <input type="text" name="authorization_digits" maxlength="4" required
                            class="w-14 text-center bg-white border border-brand-red-50 rounded px-1 py-0.5 text-brand-red font-bold focus:ring-2 focus:ring-brand-red outline-none" placeholder="____">
                        <span>(xxxx).</span>
                    </p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[480px]">
                        <thead>
                            <tr class="bg-brand-blue text-white text-sm">
                                ...
                                <th class="p-3 font-semibold text-center w-24">Rate</th>
                                <th class="p-3 font-semibold text-center w-24">Quantity</th>
                                <th class="p-3 font-semibold text-center w-32">Total</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($catalog as $item)
                                <tr class="item-row" data-rate="{{ $item['rate'] }}">
                                    ...
                                    <td class="p-3 text-center font-semibold text-gray-600">
                                        {{ $item['rate'] > 0 ? '$' . number_format($item['rate'], 2) : 'Free' }}
                                    </td>
                            ...
                            @endforeach
                            <tr class="bg-gray-50">
                                <td colspan="3" class="p-3 text-right font-bold text-brand-blue">TOTAL AMOUNT</td>
                ...
                <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
                    <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3">Signature</h3>
