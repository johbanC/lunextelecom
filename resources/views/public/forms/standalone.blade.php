...
<body class="min-h-screen bg-gray-100 bg-[radial-gradient(circle_at_top,_var(--color-brand-blue-50),_var(--color-gray-100)_60%)] p-4 md:p-10 font-sans text-gray-800">
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form id="formSubmission" method="POST" action="{{ route('public.forms.standalone.store', $template->slug) }}" class="p-5 md:p-8 space-y-6"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                @foreach ($template->fields as $field)
                    <x-form-field-input :field="$field" :value="null" :readonly="false" />
                @endforeach
            </div>
